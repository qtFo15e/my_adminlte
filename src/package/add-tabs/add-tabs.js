...
(function ($) {
    ...
    _listen = function (options) {
        // 绑定点击事件
        $(settings.monitor).on('click', '[data-addtab].js-nochild', function () {
            _click($(this));
            $.addtabs.drop();
        });
        // 根据文件url 加载iframe
        ...
        var el;
        $('body').on('dragstart.h5s', '.js-main-tab li', function (e) {
            el = $(this);
        }).on('dragover.h5s dragenter.h5s drop.h5s', '.js-main-tab li', function (e) {
            if (el == $(this)) return;
            $('.dragBack').removeClass('dragBack');
            $(this).addClass('dragBack');
            el.insertAfter($(this))
        }).on('dragend.h5s', '.js-main-tab li', function () {
            $('.dragBack').removeClass('dragBack');
        });
    ...
    $.addtabs.set = function () {
        ...
        if (typeof settings.target == 'object') {
            ...
            target = $('body').find(settings.target).length > 0 ? $(settings.target).first() : $('body').find('.js-main-tab').first();
        }
    }
    ...
    $.addtabs.add = function (opts) {
        ...
        tab_li.find('#tab_' + id).addClass('active fadeIn animated');
    ...
    $.addtabs.close = function (opts) {
        ...
        if ($("#tab_" + opts.id).hasClass('active')) {
            if ($('#tab_' + opts.id).parents('li.tabdrop').length > 0 && !$('#tab_' + opts.id).parents('li.tabdrop').hasClass('hide')) {
                $('#tab_' + opts.id).parents('.js-main-tab').find('li').last().tab('show');
            } else {
    ...
    $.addtabs.drop = function () {
        ...
        $('body').find('.js-main-tab').each(function () {
            var element = $(this);
            //检测是否已增加
